...
SCENARIO("select throws", "[select][map][operators]"){
    GIVEN("select"){
        ...
        WHEN("subscribed to throw operator"){
            THEN("the exception is not supressed"){
                REQUIRE_THROWS(rx::error<int>(std::runtime_error("throw operator"))
                    .select([](int v){return v;})
...
//                 REQUIRE_THROWS(rx::CreateObservable<int>(subscribe_throws))
//                     .select([](int v){return v;})
//                     .subscribe([](int){}));
//             }
//         }
